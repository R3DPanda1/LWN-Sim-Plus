#!/usr/bin/env python3
# Like scaling_benchmark but splits devices across AM319, MCF-LW13IO, SDM230.
import csv
import os
import time
...
from helpers import (
    ...
    cs_request, observe_seconds, scrape_metrics,
)
...
TEMPLATES = [
    {"id": 1, "name": "AM319",      "profile": "AM319 ABP"},
    {"id": 2, "name": "MCF-LW13IO", "profile": "MCF-LW13IO ABP"},
    {"id": 3, "name": "SDM230",     "profile": "SDM230 ABP"},
]
...
def resolve_ids(api_key):
    ...
    by_name = {p["name"]: p["id"] for p in profiles.get("result", [])}
    missing = [t["profile"] for t in TEMPLATES if t["profile"] not in by_name]
    if missing:
        raise RuntimeError(f"missing device profiles in ChirpStack: {missing}. Run seed.py first.")
    profile_ids = {t["id"]: by_name[t["profile"]] for t in TEMPLATES}
    return app_id, profile_ids


def set_template(tmpl_id, profile_id):
    tmpl = requests.get(f"{SIM_URL}/template/{tmpl_id}", timeout=10).json()
...
def create_mixed(count):
    ...
    for i, t in enumerate(TEMPLATES):
        ...
        remaining, batch = n, 0
        while remaining > 0:
            b = min(remaining, 5000)
            batch += 1
            r = requests.post(
                f"{SIM_URL}/create-devices-from-template",
                json={
                    "templateId": t["id"], "count": b,
                    "namePrefix": f"mix-{t['name'].lower()}-b{batch}",
                    "baseLat": 48.2077, "baseLng": 16.375,
                    "baseAlt": 0, "spreadMeters": 500,
                },
                timeout=600,
            ).json()
            created_total += r.get("created", 0)
            remaining -= b
        print(f"  {t['name']}: {n}")
    return created_total


def run_tier(count, api_key, app_id, profile_ids, ts_writer):
    window = observe_seconds(count)
    print(f"\n=== Tier {count} mixed devices (observe {window}s) ===")

    clear_simulator_devices()
    clean_chirpstack(api_key, app_id)
    for t in TEMPLATES:
        set_template(t["id"], profile_ids[t["id"]])

    created = create_mixed(count)
    if created == 0:
...
def main():
    ...
    results_path = f"mixed_results_{stamp}.csv"
    ts_path = f"mixed_timeseries_{stamp}.csv"

    print(f"Mixed-device scaling benchmark (ABP)")
    print(f"  tiers={TIERS}  send_interval={SEND_INTERVAL}s")
    print(f"  templates={[t['name'] for t in TEMPLATES]}")
    print(f"  results={results_path}")

    api_key = chirpstack_api_key("mixed-bench")
    app_id, profile_ids = resolve_ids(api_key)
    print(f"  app={app_id[:8]}")
    ...
    with open(results_path, "w", newline="") as rf, open(ts_path, "w", newline="") as tf:
        ...
        for count in TIERS:
            result = run_tier(count, api_key, app_id, profile_ids, tw)
            tf.flush()
            if result:
                rw.writerow([
